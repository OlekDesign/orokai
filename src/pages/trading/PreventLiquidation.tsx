...
function getProjectedLiquidationPrice(position: Position, addedMargin: number) {
  if (addedMargin <= 0) return position.liqPrice;
  return position.liqPrice * 1.1;
}

type PreventLiquidationLocationState = {
  asset?: Asset;
  position?: Position;
};

export function PreventLiquidation() {
  const navigate = useNavigate();
  const location = useLocation();
  const { updatePosition } = useTrading();
  const { asset, position } = (location.state as PreventLiquidationLocationState | null) ?? {};
  ...
  }, [position, depositNum, currentLiqPrice]);
              ...
              <div key={item}>
                <button
                  className={`w-full flex items-center justify-between px-2 py-3 rounded-lg transition-colors hover:bg-accent/50 ${
                    item === currency ? 'text-primary font-semibold' : 'text-foreground'
                  }`}
                  onClick={() => {
                    setCurrency(item);
                ...
                >
                  ...
                  {item === currency ? <span className="w-2 h-2 rounded-full bg-primary" /> : null}
                </button>
          ...
          <div className="space-y-6">
            <div className="rounded-lg border border-border bg-muted/20 p-4">
              ...
              >
                ...
                <defs>
                  <linearGradient id="prevent-liquidation-chart-fill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="currentColor" stopOpacity="0.2" />
                    <stop offset="100%" stopColor="currentColor" stopOpacity="0" />
                  </linearGradient>
                </defs>
                <path
                  fill="url(#prevent-liquidation-chart-fill)"
